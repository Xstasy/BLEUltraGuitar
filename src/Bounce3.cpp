// Bounce3 rewritten to support microtime based on thomasfredericks/Bounce2
...
Debouncer::Debouncer():previous_micros(0)
    , interval_micros(10)
    , state(0) {}

void Debouncer::interval(uint16_t interval_micros)
{
...
unsigned long Debouncer::currentDuration() const {
	return (micros() - stateChangeLastTime);
}

inline void Debouncer::changeState() {
	toggleStateFlag(DEBOUNCED_STATE);
	setStateFlag(CHANGED_STATE) ;
	durationOfPreviousState = micros() - stateChangeLastTime;
	stateChangeLastTime = micros();
}
